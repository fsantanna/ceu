    ...
    fun Type.aux (lin: Int, col: Int): Type {
        return when (this) {
            ...
            is Type.Tuple -> Type.Tuple(
                ...
                this.ids?.map { it.copy(lin_ = lin,col_ = col) }
            )
...
fun Type.mapScps (dofunc: Boolean, map: Map<String, Scope>): Type {
    ...
    return when (this) {
        ...
        is Type.Tuple   -> Type.Tuple(this.tk_, this.vec.map { it.mapScps(dofunc,map) }, this.ids)
        is Type.Union   -> Type.Union(this.tk_, this.vec.map { it.mapScps(dofunc,map) }, this.ids)

...
fun Stmt.setScp1s () {
    fun fx (up: Any, scp: Scope) {
        scp.scp1 = if (scp.scp1.str != "LOCAL") scp.scp1 else {
            Tk.Scp(TK.XSCP, up.localBlockScp1Id(), scp.scp1.lin, scp.scp1.col)
        }
    }
...
fun Scope.toScp2 (up: Any): Triple<Int,String?,Int?> {
    ...
    return when (this.scp1.str) { // 2xExpr.Func, otherwise no level between outer/arg/body
        "GLOBAL" -> Triple(0, null, 0)
        else -> {
            val blk = up.env(this.scp1.str)
            if (blk != null) {
                // @A, @x, ...
